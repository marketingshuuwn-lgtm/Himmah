...
const SITE_NAME = "staff-star-center"
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        // 1b. Authorize the caller for this recipient.
        // Only HR/management roles may send to arbitrary recipients; everyone
        // else may only trigger emails addressed to their own account email.
        const { data: callerRoles } = await supabase
          .from('user_roles')
          .select('role')
          .eq('user_id', user.id)

        const privileged = (callerRoles ?? []).some((r: { role: string }) =>
          ['owner', 'admin', 'hr_admin', 'accountant'].includes(r.role)
        )

        const isSelf =
          !!user.email &&
          user.email.toLowerCase() === effectiveRecipient.toLowerCase()

        if (!privileged && !isSelf) {
          console.warn('Blocked unauthorized email send attempt', {
            userId: user.id,
            templateName,
